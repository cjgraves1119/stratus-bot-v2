...
function hasMsAdvancedTierIntent(text) {
  ...
  // Advanced (A) is valid for supported switches and every MR/CW access point.
  // CW9800 is a controller rather than an access point.
  if (!/\b(MR\d+|CW(?!9800)\d+[A-Z0-9-]*|MS130|MS150|MS390|C9\d{3}|C9200L|C9300)\b/.test(upper)) return false;
...
// V3 classifier prompt (per-item product+intent, NO model SKUs). Used by classifyV3 when
// CF_QUOTE_V3_ENABLED. Kept identical across workers. Source: CF_CLASSIFIER_PROMPT_V3-draft.
...
const CF_CLASSIFIER_PROMPT_V3_ACTIVE = `${CF_CLASSIFIER_PROMPT_V3}\n\nMERAKI AP ADVANCED TIER RULE: Every MR and CW access point uses the same co-term licensing family. Enterprise is the default; explicit \"advanced\", \"ADV\", or \"advanced license\" means modifiers.tier=\"A\" and maps to LIC-MR-ADV-{term}Y. This includes Wi-Fi 7 CW models such as CW9172I; CW9800 is a controller, not an AP. Example: \"3 CW9172I advanced\" -> items=[{product:\"CW9172I\",qty:3,intent:\"normal\"}], modifiers.tier=\"A\".`;
...
// no SKU for the engine and previously fell to the Claude fallback (V3 canary 2026-06-09: 19.7s
// Sonnet turn that emitted malformed empty order URLs). Read the prior assistant clarify question,
// combine it with the tier in the reply, and return a synthesized quote request the deterministic
// engine parses directly — or null to leave the message on the existing path. Identical in both
// workers. Conservative by design: short replies only, never hijacks a fresh request, and never
// guesses the Umbrella package type (DNS vs SIG) when the reply omits it.
...
// V3 classifier — same model as classifyWithCFv2 but the V3 prompt (per-item product+intent, no
// SKUs). Returns the parsed V3 object or null (null on any failure → caller falls through to the
// existing path). Kept identical across workers; gated by CF_QUOTE_V3_ENABLED at the call sites.
async function classifyV3(userMessage, priorContext, env) {
  ...
  try {
    ...
    const result = await Promise.race([
      env.AI.run(CF_MODEL, { messages: [ { role: "system", content: CF_CLASSIFIER_PROMPT_V3_ACTIVE }, { role: "user", content: userText } ], max_tokens: 512 }),
...
- tier: "SEC" for MX "SEC"/"security"/"advanced security"; "ENT" for "ENT"/"enterprise"; "SDW" for any of "SD-WAN"/"SDW"/"SD WAN"/"sdwan"/"sd-wan"/"sd wan" (case-insensitive); "A" for every MR/CW access point and for MS130/MS150/MS390/Catalyst switches when the user requests "advanced", "advanced license", "ADV", or "adaptive policy". CW9800 is a controller, not an AP. null otherwise.
...
async function classifyWithV3Shadow(userMessage, priorContext, env) {
  ...
  try {
    ...
    const result = await Promise.race([
      env.AI.run(CF_MODEL, {
        messages: [
          { role: 'system', content: CF_CLASSIFIER_PROMPT_V3_ACTIVE },
          { role: 'user', content: userText }
        ],
        max_tokens: 512
      }),
...
function _getLicenseSkusRaw(baseSku, requestedTier) {
  ...
  // All Meraki-managed CW access points share MR co-term licensing. CW9800 was
  // returned above because it is a controller, not an access point.
  if (/^CW(?!9800)\d/.test(upper)) {
    const adv = ['A', 'ADV', 'ADVANCED'].includes(String(requestedTier || '').toUpperCase());
    if (adv) {
      return [
        { term: '1Y', sku: 'LIC-MR-ADV-1Y' },
...
function buildQuoteFromV2(v2, rawText) {
  ...
  if (hwItems.length === 0 && licItems.length > 0) {
    ...
    if (licItems.length === 1) {
      // ── F3 (seam 2026-06-10, D1 row 5240): 1/3/5 renderer invariant ──
...
function buildQuoteFromV3(v3, rawText) {
  ...
  // wrongly drops the auto-paired license. parseMessage already derives per-item
  // hardwareOnly from text — line ~4897 must not override it with the raw label. (2026-06-19)
...
// ─── Per-term standalone license family helpers ─────────────────────────────
// These identify license SKUs that the original quote renderer emitted as
// per-term isTermOptionQuote output (one URL per 1Y/3Y/5Y). When reconstructing
// prior state from the assistant URL history, pools of these SKUs must be
// promoted back to isTermOptionQuote shape so downstream revise actions
// (show_pricing, change_term, change_qty) render correctly per-term.
//
// Covers:
//   LIC-MV-{1,3,5}YR       — standalone MV camera licenses
//   LIC-MT-{1,3,5}Y        — standalone MT sensor licenses (single Y, not YR)
//   LIC-SME-{1,3,5}YR      — legacy Systems Manager licenses (substituted with the Ivanti MDM replacement at pooling)
//   LIC-(ENT|SEC|SDW)-{1,3,5}YR — tier-agnostic MR licenses
//
// Hardware-tied licenses (LIC-MX67-SEC-*YR, LIC-MS150-*, etc.) intentionally
// do NOT match — the anchored regex requires LIC- to be directly followed by
// the bare tier/family token, not a hardware model number. Those get
// regenerated by buildQuoteResponse from item+term+tier instead.
const PER_TERM_STANDALONE_RE = /^LIC-MV-[135]YR$|^LIC-MT-[135]Y$|^LIC-SME-[135]YR$|^LIC-MI-EMSC-D-1YMC-A-[135]YR$|^LIC-(ENT|SEC|SDW)-[135]YR$/;
const familyKeyFromSku = (sku) => {
  const s = String(sku || '').toUpperCase();
  if (/^LIC-MV-[135]YR$/.test(s)) return 'MV';
  if (/^LIC-MT-[135]Y$/.test(s)) return 'MT';
  if (/^LIC-SME-[135]YR$/.test(s)) return 'SME';
  if (/^LIC-MI-EMSC-D-1YMC-A-[135]YR$/.test(s)) return 'SME'; // Ivanti MDM — the SME replacement
...
function extractPriorFromAssistantUrl(content) {
  ...
  for (const url of urls) {
    ...
    for (let i = 0; i < skus.length; i++) {
      ...
      if (sku.startsWith('LIC-')) {
        // Legacy Systems Manager tokens in prior-quote URLs: substitute the
        // replacement SKU (SME is discontinued) BEFORE pooling, so every
        // downstream revise/re-render path (per-term promotion, change_term,
        // change_qty, show_pricing) works on the live catalog SKU and can
        // never re-emit a dead LIC-SME line from conversation history.
  ...
  if (items.length === 0 && licMap.size > 0) {
    ...
    // Per-term standalone family promotion: when the pooled licenses are all
    // in the same "family-agnostic" family AND span multiple term years, the
    // original render was a per-term isTermOptionQuote (one URL per term).
    // Without this promotion, a multi-term MV/MT/agnostic-MR pool collapses
    // into a single directLicenseList that renders all three terms in one
    // URL — the "what are the costs" bug on bare "N MV licenses" quotes.
    //
    // Guardrails (all four must hold):
    //   1. At least 2 license SKUs in the pool
    //   2. At least 2 distinct term years present (1, 3, 5)
    //   3. Every SKU matches PER_TERM_STANDALONE_RE
    //   4. Every SKU maps to the SAME family key (prevents mixed-family
    //      collapse like [LIC-MV-3YR, LIC-ENT-3YR] from being promoted)
    //
    // Multi-family pools stay as directLicenseList below. Single-term pools
    // fall through to directLicense for the same reason.
    const perTermTermsSet = new Set();
    for (const l of licList) {
    ...
    const isAllPerTermStandalone =
      licList.length >= 2 &&
      perTermTermsSet.size >= 2 &&
      licList.every(l => PER_TERM_STANDALONE_RE.test(String(l.sku || '').toUpperCase())) &&
      familyKeys.size === 1;
...
function clauseRequestedTier(clauseText, itemSku) {
  const t = String(clauseText || '').toUpperCase();
  const sku = String(itemSku || '').toUpperCase();
  const isSwitch = /^(MS130|MS150|MS390|C9\d{3}|C9200L|C9300)/.test(sku);
  const isMxZ = /^(MX|C8111|C8121|C8455|Z\d)/.test(sku);
  const isMr = /^MR/.test(sku);
  const isCw = /^CW/.test(sku);
  const isMrCw = isMr || isCw;
...
function assignClauseTier(items, upper) {
  ...
  for (const item of items) {
    ...
    const clause = clauses.find((c) => item.position >= c.start && item.position < c.end);
    if (!clause) continue;
    const tier = clauseRequestedTier(clause.text, item.baseSku || item.sku);
    if (tier) item.requestedTier = tier;
  }
}
...
function parseMessage(text) {
  ...
  // License-only, model-agnostic (like MR/MV/MT). Maps "Systems Manager",
  // "SME", or bare "LIC-SME" to LIC-SME-{term}YR. Pure SME requests return
...
function buildQuoteResponse(parsed) {
  ...
  if (parsed.isTermOptionQuote && parsed.items) {
      ...
      for (const [tierKey, label] of tierFamilies) {
        ...
        for (const term of ['1YR', '3YR', '5YR']) {
          ...
          if (matching.length > 0) {
            ...
            // Exact-group pricing: only the SKUs in THIS tier+term URL, not
            // the broader parsed state. Strict boolean check to prevent any
            // truthy-but-not-true value from flipping pricing on.
            if (parsed.showPricing === true) {
              lines.push(buildPricingBlock(matching, true).trim());
            }
          }
        }
        lines.push('');
    ...
    for (const [term, skus] of Object.entries(termGroups)) {
      if (skus.length > 0) {
        ...
        // Exact-group pricing: price only the SKUs in this term's URL.
        if (parsed.showPricing === true) {
  ...
  for (let { baseSku, qty, hardwareOnly: itemHardwareOnly, licenseOnly: itemLicenseOnly, _v3PreLicense, smeReplaced: itemSmeReplaced, requestedTier: itemRequestedTier } of parsed.items) {
    ...
    if (eol && replacement) {
      const eolItem = { baseSku, qty, replacement, eol: true, hardwareOnly: itemHardwareOnly, licenseOnly: itemLicenseOnly, requestedTier: itemTier };
...
### APs (MR + CW) — MR supports Advanced; default MR and CW use generic ENT
- MR APs with no explicit Advanced request → LIC-ENT-1YR, LIC-ENT-3YR, LIC-ENT-5YR (note: -YR suffix)
- MR APs explicitly requested as Advanced/ADV → LIC-MR-ADV-1Y, LIC-MR-ADV-3Y, LIC-MR-ADV-5Y (note: -Y suffix)
- CW9 APs → LIC-ENT-1YR, LIC-ENT-3YR, LIC-ENT-5YR (note: -YR suffix), even when Advanced is requested
    ...
    if (request.method === 'POST' && url.pathname === '/webhook') {
      ...
      ctx.waitUntil((async () => {
        ...
        try {
          ...
          // ── Tier-clarify continuation (deterministic) ──
          // A bare tier reply ("Essentials") after the Duo/Umbrella tier question re-enters the
          // deterministic engine instead of falling to the Claude fallback (V3 canary 2026-06-09:
          // Sonnet took 19.7s and emitted malformed empty order URLs for this exact flow).
      ...
      try {
        ...
        if (deepSeekRequested) {
          ...
          const requestBody = isGemma
            ...
            : { messages: [{ role:'system', content: systemPrompt }, { role:'user', content: userText }], max_tokens: classifierMaxTokens };
