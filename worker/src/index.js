...
function hasMsAdvancedTierIntent(text) {
  ...
  // Advanced (A) is valid for supported switches, MR APs, and Meraki-managed
  // CW916x APs. Other CW families retain their existing licensing rules.
...
- tier: "SEC" for MX "SEC"/"security"/"advanced security"; "ENT" for "ENT"/"enterprise"; "SDW" for any of "SD-WAN"/"SDW"/"SD WAN"/"sdwan"/"sd-wan"/"sd wan" (case-insensitive); "A" for MR access points, Meraki-managed CW916x access points, and MS130/MS150/MS390/Catalyst switches when the user requests "advanced", "advanced license", "ADV", or "adaptive policy". Other CW families retain their existing rules. null otherwise.
...
function clauseRequestedTier(clauseText, itemSku) {
  ...
  if (!/\bADVANCED\s+SECURITY\b/.test(t) && /\b(ADVANCED|ADV)\b/.test(t)) {
    if (isSwitch || isMr || /^CW916\d/.test(sku)) return 'A';
    if (isCw) return 'ENT';
  }
  return null;
}
