...
test('active Webex V2 adapter preserves MR Advanced and excludes CW', () => {
  assert.equal(worker.hasMsAdvancedTierIntent('3 MR44 advanced'), true);
  assert.equal(worker.hasMsAdvancedTierIntent('3 CW9164I advanced'), false);
  assert.equal(worker.normalizeRequestedTier('A', '3 MR44 advanced'), 'A');
  assert.equal(worker.normalizeRequestedTier('A', '3 CW9164I advanced'), null);
...
test('active V3 prompts teach MR Advanced while keeping CW Enterprise-only', () => {
